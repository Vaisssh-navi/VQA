import json
import os
from pathlib import Path
from PIL import Image
import torch
import re

from mplug_owl2.model.builder import load_pretrained_model
from mplug_owl2.mm_utils import process_images, tokenizer_image_token, KeywordsStoppingCriteria
from mplug_owl2.conversation import conv_templates
from mplug_owl2.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN


# ===================== PATHS =====================
BASE_DIR = Path("/home/ee/btech/ee1221719/mmc-work/mPLUG-Owl/mPLUG-Owl2")
BENCH_DIR = BASE_DIR / "MMC-DATA" / "MMC-Benchmark"

MQA_JSONL = BENCH_DIR / "mmc_benchmark_mqa_text.jsonl"
MQA_IMG_DIR = BENCH_DIR / "mqa_images/"

OUTPUT_FILE = BASE_DIR / "pred_mmc_mqa.jsonl"
# =================================================


def load_model():
    print("[INFO] Loading mPLUG-Owl2 model...")
    model_path = "MAGAer13/mplug-owl2-llama2-7b"

    tokenizer, model, image_processor, ctx_len = load_pretrained_model(
        model_path, None, model_path, device="cuda:0"
    )

    model.eval()
    print("[INFO] Model loaded.")
    return tokenizer, model, image_processor


# ------------------ NORMALIZATION UTILS ------------------

def normalize(text: str):
    """Normalize text for comparison."""
    if not isinstance(text, str):
        return ""
    t = text.lower().strip()

    # remove brackets, parentheses
    t = re.sub(r"[\(\)]", "", t)

    # remove prefix like "a)", "b.", "c)"
    t = re.sub(r"^[a-d]\s*[\)\.:-]*\s*", "", t)

    # remove multiple spaces
    t = " ".join(t.split())
    return t


def is_correct_mcq(pred, gt):
    """MCQ correctness using relaxed rules."""
    p = normalize(pred)
    g = normalize(gt)

    # exact match
    if p == g:
        return True

    # if gt contains keyword, check partial match
    if g in p:
        return True

    # match choices like "c" or "c)"
    if len(g) == 1 and g in p:
        return True

    return False


def is_correct_general(pred, gt):
    """General correctness for topic/chart classification."""
    p = normalize(pred)
    g = normalize(gt)

    if p == g:
        return True
    if g in p:
        return True

    return False


# ------------------ INFERENCE ------------------

def infer_one(tokenizer, model, image_processor, img_path, instruction):
    image = Image.open(img_path).convert("RGB")
    max_edge = max(image.size)
    image = image.resize((max_edge, max_edge))

    img_tensor = process_images([image], image_processor)
    img_tensor = img_tensor.to(model.device, dtype=torch.float16)

    query = instruction.strip()

    conv = conv_templates["mplug_owl2"].copy()
    conv.append_message(conv.roles[0], DEFAULT_IMAGE_TOKEN + query)
    conv.append_message(conv.roles[1], None)

    prompt = conv.get_prompt()

    input_ids = tokenizer_image_token(
        prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt"
    ).unsqueeze(0).to(model.device)

    # --- Prevent OOM ----
    if input_ids.shape[1] > 3500:
        input_ids = input_ids[:, -3500:]
    # ---------------------

    stopper = KeywordsStoppingCriteria([conv.sep2], tokenizer, input_ids)

    with torch.inference_mode(), torch.cuda.amp.autocast(dtype=torch.float16):
        output_ids = model.generate(
            input_ids,
            images=img_tensor,
            do_sample=False,
            temperature=0.0,
            max_new_tokens=64,
            stopping_criteria=[stopper],
        )

    out = tokenizer.decode(output_ids[0, input_ids.shape[1]:],
                           skip_special_tokens=True).strip()
    return out


# ------------------ MAIN LOOP ------------------

def main():
    tokenizer, model, image_processor = load_model()

    print(f"[INFO] Reading MQA dataset: {MQA_JSONL}")
    fout = open(OUTPUT_FILE, "w")

    count = 0

    with open(MQA_JSONL, "r", encoding="utf-8") as f:
        for line in f:
            item = json.loads(line)

            img_id = item["image_id"]   # FIXED HERE
            instruction = item["instruction"]
            gt_label = item["label"]
            task = item.get("task", "")

            img_path = MQA_IMG_DIR / img_id

            pred = infer_one(tokenizer, model, image_processor, img_path, instruction)

            # Decide correctness method
            if ")" in gt_label or "a)" in instruction.lower():
                correct = is_correct_mcq(pred, gt_label)
            else:
                correct = is_correct_general(pred, gt_label)

            out = {
                "image_id": img_id,
                "instruction": instruction,
                "gt": gt_label,
                "pred": pred,
                "correct": correct,
                "task": task
            }

            fout.write(json.dumps(out) + "\n")

            count += 1
            if count % 20 == 0:
                print(f"[INFO] Processed {count} samples...")

    fout.close()
    print("[DONE] Saved:", OUTPUT_FILE)


if __name__ == "__main__":
    main()

