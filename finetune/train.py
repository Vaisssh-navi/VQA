...
from mplug_owl2.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    bits: int = field(
        default=4,
        metadata={"help": "How many bits to use (4, 8, or 16)."}
    )
...
def train():
    ...
    model.config.tune_visual_abstractor = getattr(model_args, "tune_visual_abstractor", training_args.tune_visual_abstractor)
    ic(training_args.tune_visual_abstractor)

    # ---- Q-LoRA vs full finetune requires_grad logic ----
    if training_args.lora_enable and training_args.bits in [4, 8]:
        # Q-LoRA mode: freeze base, train only LoRA + selected modules
        ...
        # Original behavior (no Q-LoRA / full precision)
